...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <!--***************************************************************************
  ...
  <xsl:template name="timezones">
      ...
      <xsl:variable name="IST-timestamp" select="adjust-dateTime-to-timezone($UTC-timestamp, xsi:dayTimeDuration($IST-offset))"/>
      
 
      <meeting UTCtime="{$UTC-timestamp}" >
          <xsl:variable name="ist-offset" select="format-dateTime($IST-timestamp, '[ZN,*-3]', (), (), 'uk')"/>
          <datetime location="Mubai" timezone="IST" UTCoffset="{$ist-offset}">
              <xsl:value-of select="format-dateTime($IST-timestamp, '[D] [MN,*-3] [Y] [h]:[m01][PN,*-2]', (), (), 'uk')"/>
          </datetime>
       </meeting>
  </xsl:template>
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <BR/>
                  	<img src ="https://www.grid-tools.com/wpcms/wp-content/uploads/2014/04/orasi-2-e1397659616153-193x171.png" style="margin:-20px 0 0 0;"/>
                  	
                  <xsl:variable name="currDT" select="format-dateTime((suite[@name != 'Default Suite']/@started-at), '[D01]-[M01]-[Y0001]  [H01]:[m01]:[s01] [P]' )"/>	 
    				<h1>OrangeHRM Demo</h1>
    			  <xsl:value-of select = "$currDT" />
    			  
    			  
               <!--  <xsl:value-of select="format-dateTime((suite[@name != 'Default Suite']/@finished-at), '[D01]-[M01]-[Y0001]  [H01]:[m01]:[s01] [P]' )"/>  --> 	
				...
				<!-- 		 <xsl:value-of select="current-time()"/>	 -->
				 ...
				 <!--    <xsl:variable name="currDT" select="current-dateTime()"/>
				    ...
				    <xsl:template match="/*">
				        <mytag starttime="{$currDT}" endtime="{$currDT + dayTimeDuration(concat('PT',@duration,'S'))}"/>
				    </xsl:template>			 
									 -->	 
